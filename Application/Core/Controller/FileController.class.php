...
class FileController extends Controller
{
    ...
    /* 下载房源图片，将该房源最多的9张图片打包然后下载*/
    /*
     *  打包采用zip模式
     *@param $oid intval 房源ID
     *@return 下载动作
     */

    public function downloadObject($id = null)
    {
        if (empty($id) || !is_numeric($id)) {
            $this->error('参数错误！');
        }
        ...
        $FILE = D('Home/File');
        $pics = D('object_pic')->where(array('oid'=>$id))->getField('pid',true);
        $name =  D('Object')->where(array('id'=>$id))->field('lname,nickname')->alias('a')
                  ->join('__MEMBER__  b on a.uid = b.uid')
                  ->find();
        $zipname =  $name['lname'].'_'.$name['nickname'].'_'.date('Y-m-d').'.zip';

        if(!$pics){
            $this->error('没有文件');
        }

        $FILE->downloadObjectPic($pics,$zipname);
    }
