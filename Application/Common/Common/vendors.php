...
/*
 * 改装后的get_city_by_ip 函数，返回一个城市字符串
 */
function get_city_by_lee_ip($ip){
    $url = "http://ip.taobao.com/service/getIpInfo.php?ip=" . $ip;
    $ipinfo = json_decode(file_get_contents($url));
    if ($ipinfo->code == '1') {
        return false;
...
function sendSMS($mobile, $content, $time = '', $mid = '')
{
    ...
    (
        //'uid' => $uid, //用户账号
        'account' => iconv('GB2312','GB2312',$uid), //用户账号
        //'pwd' => strtolower(md5($pwd)), //MD5位32密码
        'pswd' => iconv('GB2312','GB2312',$pwd), //MD5位32密码
        'mobile' => $mobile, //号码
        'msg' => iconv('GB2312','GB2312','验证码:123123'), //内容 如果对方是utf-8编码，则需转码iconv('gbk','utf-8',$content); 如果是gbk则无需转码
//        'time' => $time, //定时发送
 //       'mid' => $mid, //子扩展号
  //      'encode' => 'utf8',
    );
...
function chuanglan_postSMS($mobile,$msg){

	    $account = modC('SMS_UID', '', 'USERCONFIG');
	    $pswd = modC('SMS_PWD', '', 'USERCONFIG');
	    $url = modC('SMS_HTTP', '', 'USERCONFIG');
	  

        $post_data = array();
		$post_data['account'] = iconv('GB2312', 'GB2312',$account);
		$post_data['pswd'] = iconv('GB2312', 'GB2312',$pswd);
