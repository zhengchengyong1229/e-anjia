...
/*
 * 为完善热门商圈而设计的函数
 * $bid 商圈id，存在于object表中,根据该id，追到省
 *
 * autor: smartymoon
 *
 */

function get_whole_address($bid){

    $data = M('cbd')->alias('a')->where(array('a.id'=>$bid))->field('a.id as cbd,b.id as district,c.id as city,d.id as province')
                    ->join('__CBD__ b on a.pid = b.id','left')
                    ->join('__DISTRICT__ c on b.pid = c.id','left')
                    ->join('__DISTRICT__ d on c.upid = d.id','left')
                    ->find();
    return $data;
...
function get_whole_address_by_uid($uid){
    $data = M('property')->alias('a')->where(array('a.uid'=>$uid))->field('a.id as property,b.id as cbd, c.id as district,d.id as city,e.id as province')
                         ->join('__CBD__ b on a.pid = b.id','LEFT')
                         ->join('__CBD__ c on b.pid = c.id','LEFT')
                         ->join('__DISTRICT__  d on d.id = c.pid','LEFT')
                         ->join('__DISTRICT__  e on d.upid = e.id','LEFT')
                         ->find();
    return $data;
