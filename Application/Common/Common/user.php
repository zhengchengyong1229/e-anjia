...
//从auth_group_accse 获得权限id
/*
 *@param $uid 用户id
 *@rutrun intval 1 普通用户,2 认证用户，3普通机构，4认证机构
 */
function get_user_auth_group($uid){
    $uid = $uid?$uid:is_login();
    //获得权限表
    $group_ids = D('auth_group_access')->where(array('uid'=>$uid))->getField('group_id');
    return $group_ids;
}
