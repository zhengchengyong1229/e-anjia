...
class MemberController extends Controller
{
   ...
   /* handl-ajxa 退出登陆*/
    public function moblogout()
    {
        if (is_login()) {
                D('Member')->logout();
                $this->ajaxReturnHandle(1,'退出登陆成功',U('home/index/index'));
        } else {
                $this->ajaxReturnHandle(0,'您没有登陆');
        }
    }
    ...
    public function doSendVerify($account, $verify, $type)
    {
        switch ($type) {
            case 'mobile':
                ...
                $res = chuanglan_postSMS($account,$content);
                return $res;
                break;
            case 'email':
                //发送验证邮箱
    ...
    public function saveAvatar()
    {
        ...
        A('Ucenter/UploadAvatar', 'Widget')->mobcropPicture($aUid, $aCrop, $aExt);
    ...
    protected function initialShuo($uid){
        $city_info = session('user_city');
        $city     = $city_info['city_id'];
        $province  = D('district')->where(array('id'=>$city))->getField('upid');
