...
class ConfigController extends BaseController
{
     ...
     * 
     */
    public function mobavatar()
    {
        if(IS_POST){

                   if(((bool)I('post.ext', '', 'op_t'))){
                      $avatar_res =   A('Ucenter/member')->saveAvatar();
                   }

                   $nickname = I('post.nickname','','op_t');

                   if(utf8_strlen($nickname)<4){
                       $this->error('昵称长度不得少于4');
                   }

                   //先检测名字是否被占用
                   if(!$if_exsits = D('member')->where(array('nickname'=>$nickname,'uid'=>array('neq',is_login())))->getField('uid')){
                   //    echo 'haha';
                      $res = D('member')->where(array('uid'=>is_login()))->save(array('nickname'=>$nickname));
                   }
    ...
    public function apply($license = 0){
        //添加申请
        $group_id = get_user_auth_group();
        $d_apply      = D('Apply');
        if(IS_POST){
            //已认证用户访问此处提示正在认证或者认证成功
            if(in_array($group_id,array(2,4))){
                //已经认证成功
                $this->ajaxReturnHandle(1,'您已经认证成功');

            }else{
                //取出未禁用的申请
                $res = $d_apply->where(array('uid'=>$this->mid,'status'=>array('neq',-1)))->find();
                ...
                $data['to_auth'] = $to_auth;

                if(!$res){
                    //表示未申请,新增一条申请记录

                    $res = $d_apply->add($data);

                    if($res){
                        $this->ajaxReturnHandle(1,'您已经成功提交申请，请等待审核');
                    }else{
                        $this->ajaxReturnHandle(0,'提交认证失败');
                    }

                }else if($res['status'] == 2){
                    //表示待审核,提示待审核
                    $this->ajaxReturnHandle(0,'正在审核中');
                }else if($res['status'] == 0){
                    //表示拒绝审核,修改审核资料，若修改成功,将状态改为2，即为待审核
                    $data['status'] = 2;
                    $res = $d_apply->where(array('uid'=>$this->mid))->save($data);
                    if($res){
                        $this->ajaxReturnHandle(1,'重新提交申请成功,请等待审核');
                    }else{
                        $this->ajaxReturnHandle(0,'重新提交申请失败');
                    }
                } 
            }
        }else{
            //展示页面
            /*
             * 获得当前用户的身份信息，个人还是机构,若已认证，提示恭喜，若未认证，展示申请页面
             * 根据 group_id 在视图中控制样式
             */
              $this->_setTab('apply');
              $apply = $d_apply->alias('a')->where(array('a.uid'=>$this->mid,'a.status'=>array('neq',-1)))->field('b.identity,a.status,a.license')
                        ->join('__BROKER__ b on a.uid = b.uid','left') 
                        ->find();


              $identity_table = array(1=>'经纪人',2=>'中介',3=>'楼盘');
              $apply['shenfen'] = $identity_table[$apply['identity']];
